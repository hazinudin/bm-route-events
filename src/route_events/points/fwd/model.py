...
class RouteFWD(RoutePointEvents):
    ...
    def from_excel(
        ...
        schema = RouteEventsSchema(
            file_path=config_path, ignore_review_err=ignore_review
        )

        df_str = (
            pl.read_excel(excel_path, engine="calamine", infer_schema_length=None)
            .rename(str.upper)
            .cast(pl.String)
        )
