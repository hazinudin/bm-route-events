...
class BridgeInventory(object):
    ...
    def from_invij_popup(
        cls, data: dict, sups_key="BANGUNAN_ATAS", ignore_review_err=False
    ):
        """
        Load popup data INVIJ input to BridgeInventory object.
        """
        profile_schema = InventoryProfileSchema(ignore_review_err, popup=True)
        sups_schema = SuperstructureSchema(ignore_review_err, popup=True)

        profile_data = profile_schema.model.model_validate(data).model_dump(
            by_alias=True
        )
        ...
        profile_data = invij_model.model_dump(
            exclude=[sups_key, "MODE", "VAL_HISTORY"], by_alias=True
        )  # Inventory profile

        sups_data = invij_model.model_dump(include=sups_key, by_alias=True)[
            sups_key
        ]  # Superstructure data
        ...
        sups = Superstructure.from_invij_popup(
            inv.id, inv.inv_year, sups_data, validate=False
        )
    ...
    def from_invij(cls, data: dict, ignore_review_err=False):
        ...
        profile_data = invij_model.model_dump(
            exclude=["BANGUNAN_ATAS", "BANGUNAN_BAWAH", "MODE", "VAL_HISTORY"],
            by_alias=True,
        )  # Inventory profile
        sups_data = invij_model.model_dump(include="BANGUNAN_ATAS", by_alias=True)[
            "BANGUNAN_ATAS"
        ]  # Superstructure data
        subs_data = invij_model.model_dump(include="BANGUNAN_BAWAH", by_alias=True)[
            "BANGUNAN_BAWAH"
        ]
        ...
        if len(sups_data) != 0:
            # Initiate Superstructure object
            sups = Superstructure.from_invij(
                inv.id, inv.inv_year, sups_data, validate=False
            )
            inv.add_superstructure(sups)
        ...
        if len(subs_data) != 0:
            ...
            subs = Substructure.from_invij(
                inv.id, inv.inv_year, subs_data, validate=False
            )
    ...
    def __init__(self, inv_data: pa.Table, state: str = None):
        ...
        self._inv_state_col = 'INVENTORY_STATE'
    ...
    @property
    def inventory_state(self) -> Literal["DETAIL", "POPUP", "VALIDASI VERIFIKASI"]:
        """
    ...
    def add_superstructure(self, obj: Superstructure, replace=False):
        ...
        if type(obj) != Superstructure:
            raise TypeError(
                f"Could only set superstructure with Superstructure object, not with {type(obj)}."
            )
    ...
    @classmethod
    def from_sups_only_update(
        cls, data: dict, existing_inv: "BridgeInventory", ignore_review_err=False
    ):
        """
        ...
        class SupsModel(sups_schema.model):
            ...
            INV_YEAR: int = existing_inv.inv_year
        ...
        # 3. Create partial Superstructure
        partial_sups = Superstructure.from_invij_sups_only(
            bridge_id, existing_inv.inv_year, sups_data, validate=False
        )
        ...
        profile_df = existing_inv.pl_df.with_columns(
            [
                pl.lit(profile_model["BRIDGE_LENGTH"]).alias("BRIDGE_LENGTH"),
                pl.lit(profile_model["CONS_YEAR"]).alias("CONS_YEAR"),
                pl.lit(profile_model["MAIN_SPAN_TYPE"]).alias("MAIN_SPAN_TYPE"),
            ]
        )

        # 6. Construct BridgeInventory
        inv = cls(profile_df.to_arrow(), state=existing_inv.inventory_state)

        # 7. Attach merged superstructure
        inv.add_superstructure(merged_sups, replace=True)

        # 8. Attach existing substructure
    ...
    def get_span_numbers(
        self, span_type: Literal["utama", "kanan", "kiri", "all"], seq: int = 1
    ):
        """
