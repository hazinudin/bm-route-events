...
class BridgeInventoryValidation(object):
    ...
    def main_span_num_check(self):
        ...
        if not self._inv.get_span_numbers('utama')[self._inv._sups._span_num_col].min() != 1:
    ...
    def compare_total_span_length_to_inv_length_check(self):
        ...
        if not isclose(total_main_span_len, self._inv.length):
            msg = f"Total panjang bentang utama ({total_main_span_len}m) tidak sama dengan panjang data inventori ({self._inv.length})"
            self._result.add_message(msg, 'review', 'review')
    ...
    def span_subs_count_check(self):
        ...
        for span in spans:
            ...
            if span_count != (subs_count + 1):
                msg = f"""
                Bentang {span[0]}/{span[1]} memiliki jumlah bangunan bawah yang tidak cocok dengan jumlah bangunan atas, 
                yaitu {span_count} bangunan atas dan {subs_count} bangunan bawah.
                """
