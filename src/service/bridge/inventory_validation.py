...
from route_events.bridge.inventory.profile.model import SupsOnlyProfileSchema
...
class BridgeInventoryValidation(object):
    def __init__(
        ...
        **kwargs,
        ...
        self._bm_repo = BridgeMasterRepoDB(sql_engine=sql_engine)

        # BridgeInventoryRepo using GeoDatabase
        self._repo = BridgeInventoryRepo(sql_engine=sql_engine)
        ...
        try:
            ...
            if sups_only:
                ...
                id_jbt = data.get("ID_JBT")
                if not id_jbt:
                    self._result = ValidationResult("failed_request")
                    self._result.add_message("ID Jembatan harus diisi.", "rejected")
                    return
                ...
                try:
                    ...
                    if self.get_status() == "rejected":
                        ...
                        # Re-parse with review errors ignored
                        self._inv = BridgeInventory.from_sups_only_update(
            ...
            if self.get_status() == "rejected":
                ...
                if kwargs.get("popup"):
                    ...
                    self._inv = BridgeInventory.from_invij(data, ignore_review_err=True)

        # LRS object
        if not sups_only:
            try:
                self._lrs = LRSRoute.from_feature_service(
                    lrs_grpc_host, route=self._inv.linkid
                )
            except:
        ...
        # BridgeInventory with the same bridge ID (skip re-fetch for sups_only, already set)
        if not sups_only:
            ...
            if len(self._available_inv_years) == 0:
                ...
                self._current_inv = self._repo.get_by_bridge_id(
                    self._inv.id, inv_year=max(self._available_inv_years)
                )
        ...
        if self._bm is None:
            self._result.add_message(
                "Jembatan belum memiliki data pokok jembatan.", "rejected"
            )

        if (not sups_only) and (self._lrs is None):
            self._result.add_message(
                f"Ruas {self._inv.linkid} bukan merupakan jalan nasional.", "rejected"
            )

        if (
            (not sups_only)
            and (self._current_inv is None)
            and (self.validation_mode == "UPDATE")
        ):
            self._result.add_message(
                f"Jembatan {self._inv.id} tidak tersedia di untuk diupdate.", "rejected"
            )
    ...
    def has_sups_check(self, check_elements=True):
        """
        Check if the inventory data has superstructure data.
        """
        if self._inv.sups is None:
            self._result.add_message(
                "Jembatan tidak memiliki data bangunan atas.", "error"
            )
        else:
            if check_elements and (self._inv.sups.elements is None):
                self._result.add_message(
                    "Jembatan tidak memiliki elemen bangunan atas", "error"
                )
    ...
    def has_subs_check(self):
        ...
        if self._inv.subs is None:
            self._result.add_message(
                "Jembatan tidak memiliki data bangunan bawah.", "error", "force"
            )
        else:
            if self._inv.subs.elements is None:
                self._result.add_message(
                    "Jembatan tidak memiliki elemen bangunan bawah", "error"
                )

        return self

    def base_check(self, validate_length: bool = True, validate_width: bool = True):
        """
    ...
    def base_check_sups_only(
        ...
        """
        # Does not check the elements since the payload does not have elements
    ...
    def update_check(self):
        ...
        if (
            (self._inv.subs is None)
            and (self._current_inv.subs is not None)
            and (self.get_status != "error")
        ):
            try:
                self._inv.sups.add_substructure(self._current_inv.subs)
            except ValueError:
                self._result.add_message(
                    "Bangunan atas tidak cocok dengan bangunan bawah inventori jembatan yang sudah ada.",
                    "error",
                )
    ...
    def main_span_num_check(self):
        ...
        if (
            self._inv.get_span_numbers("utama")[
                self._inv._sups._span_num_col
            ].list.min()[0]
            != 1
        ):
    ...
    def other_span_num_exist_in_main_span_check(self):
        ...
        errors = (
            df.with_columns(
                REF=df.filter(pl.col(self._inv.sups._span_type_col) == "UTAMA")[
                    self._inv.sups._span_num_col
                ].first()
            )
            .filter(
                pl.col("REF")
                .list.set_difference(self._inv.sups._span_num_col)
                .list.len()
                != 0
            )
            .rows(named=True)
    ...
    def span_seq_check(self):
        ...
        for span in result:
            if not result[
                span
            ]:  # Error, the span does not have correct sequence number
    ...
    def compare_total_span_length_to_inv_length_check(self):
        ...
        if not bool(isclose(total_main_span_len, self._inv.length)):
            if (self._inv.length > total_main_span_len) and (
                (self._inv.length - total_main_span_len) > 1
            ):
    ...
    def _span_no_changes(self, column: str, column_alias: str):
        ...
        diff = (
            joined.filter(pl.col(column) != pl.col(column + "_right"))
            .select(
                [
