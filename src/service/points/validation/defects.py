...
from ...photo.client import SurveyPhotoStorage
from route_events import RouteDefects, RouteDefectsRepo, LRSRoute, SurveyPhoto
...
class RouteDefectsValidation(RoutePointEventsValidation):
    ...
    def validate_excel(
        ...
        sql_engine: Engine,
        lrs: LRSRoute,
        photo_storage: SurveyPhotoStorage,
    ...
    def survey_photos(self) -> List[SurveyPhoto]:
        ...
        if self._photos is None:
            self._photos = [
                SurveyPhoto(**_)
                for _ in self._events.pl_df.join(
                    self.df_lrs_mv,
                    on=[
    ...
    def survey_photo_url_check(self):
        """
        Validate photo ID which is stored in the repository.
        """
        for invalid in self._storage.validate_photos_url(
            self.survey_photos, return_invalid=True
        ):
            msg = f"{invalid.photo_id} bukan merupakan photo ID valid atau gambar tidak ditemukan."
    ...
    def surface_type_check(self):
        ...
        surf_types = (
            ...
            .unique("surf_type")
            .select(["surf_type", "category"])
        )

        errors = (
            segments_points_join(
                segments=self.rni,
                points=self._events,
                how="inner",
            ...
            .filter(pl.col("category").ne(pl.col("category" + suffix)))
            .select(
                msg=pl.format(
        ...
        self._result.add_messages(errors, "review", "review")
    ...
    def damage_surface_type_check(self):
        ...
        errors = (
            ...
            .filter(
                ...
                .str.starts_with("AS")
                .and_(pl.col("category").eq("paved"))
                .and_(pl.col(self._events._surf_type_col).eq(21))
                | pl.col(self._events._defects_type_col)
                .str.starts_with("RG")
                .and_(pl.col("category").eq("paved"))
                .and_(pl.col(self._events._surf_type_col).ne(21))
    ...
    def damage_severity_check(self):
        ...
        self._result.add_messages(error, "error")
