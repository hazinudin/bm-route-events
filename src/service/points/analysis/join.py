...
def segments_points_join(
    ...
    # Segment DataFrame
    sdf = segments.pl_df.select(segment_id_col + segment_select)

    # Point DataFrame
    pdf = points.pl_df.select(point_id_col + point_select)

    # Aggregation for segment data
    if segments_agg is not None:
        sdf = sdf.group_by(csegment_id_col).agg(*segments_agg)

    # Aggregation for points data
    if points.lane_data and (points_agg is not None):
        pdf = pdf.group_by([points._linkid_col, points._sta_col]).agg(*points_agg)
    ...
    if how == "inner":
        ...
        if (points.lane_data and (points_agg is not None)) and (
            segments_agg is not None
        ):
            return (
                pdf.lazy()
                .join(
